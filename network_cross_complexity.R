...
netindex.file <- "network_cross domain_property.csv"
...
netindex <- netindex[rownames(treat), ]
...
plot.data <- subset(plot.data, variable %in% c(
...
plot.data$variable <- factor(plot.data$variable, levels = c(
  "Link number", "Average degree",
  "Linkage density"
))
...
divindex <- result[, 1:3, drop = F]
treat.used <- result[, -c(1:3)]
...
write.csv(divs1, paste0("complexity", "_", prefix, "_", "P value.csv"))
